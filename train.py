...
class TrainNetWrapper(nn.Cell):
    ...
    def __init__(self, backbone):
        ...
        if False:
            milestone = [2, 4] + list(range(10, 100, config.lr_decay_step))
            initial_lr = config.initial_lr
            learning_rates = [initial_lr, 2.0 * initial_lr] + [math.pow(2.*initial_lr, (i-8)//config.lr_decay_step ) 
                                                                for i in range(10, 100, config.lr_decay_step)]
        ...
        self.loss_train_net = nn.TrainOneStepCell(loss_net, optimizer)
...
if __name__ == '__main__':
    if config.device == 'GPU': os.environ['CUDA_VISIBLE_DEVICES'] = '0' # select GPU if necessary
